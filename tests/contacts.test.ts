...
import { activeContactDisabledReason, contactDayFlag, npcAllianceFlag, performActiveContact } from '../game/engine/contacts.ts';
...
test('没有电力时依次使用电池或手摇发电，不会凭空完成通讯', () => {
  ...
  assert.equal(crankResult.state.stats.stamina, crank.stats.stamina - 10);
});
...
test('信任达标后可主动邀请结盟，四种职业能力产生实际效果', () => {
  ...
  assert.equal(siegeMitigation(chenAllied), baseMitigation + 3);
  ...
  assert.equal(dangerRisk(qiuAllied, 40).risk, baseRisk - 5);
  ...
  assert.equal(repaired.state.shelter.integrity, 62);
  ...
  assert.equal(alliedNight.stats.health, ordinaryNight.stats.health + 4);
});
