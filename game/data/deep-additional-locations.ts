...
const PAN_HARDWARE: DeepLocation = {
  ...
  scenes: [
    ...
    { id: 'basement', name: '地下库房', connections: ['aisles', 'backyard'], text: '台阶下有浓重机油味。地面用粉笔划分出“客户寄存”和“报废回收”，最深处的应急柜被水汽锈成暗红色。', targets: [{ id: 'emergency-locker', name: '应急维修柜', observation: '柜门变形但没有完全锈死。标签注明里面应有电池、铜线和一套工具。', options: [
      { id: 'restore', label: '清理铰链后完整开柜', hint: '70分钟 · 体力 -8 · 多用钳 · 工具经验 +3', result: '你刮掉锈层，逐次松动铰链。柜门最终按原方向打开，里面东西没有被挤坏。', minutes: 70, stamina: 8, requirements: [{ item: 'multitool' }], loot: { toolkit: 1, batteries: 2, 'copper-wire': 1 }, skillXp: { toolUse: 3 } },
      { id: 'hatchet', label: '沿锈穿处劈开薄板', hint: '30分钟 · 体力 -14 · 手斧 · 风险较高', result: '锈板碎片落了一地。你只来得及拿走外层物资，地下室深处随即传来拖动声。', minutes: 30, stamina: 14, requirements: [{ item: 'hatchet' }], loot: { batteries: 1, 'copper-wire': 1 }, skillXp: { toolUse: 2 }, danger: 36 },
    ] }] },
  ],
};
...
const METRO_LINE4: DeepLocation = {
  ...
  scenes: [
    ...
    { id: 'platform', name: '四号线站台', connections: ['concourse', 'tunnel'], text: '屏蔽门停在半开状态，隧道风断断续续吹进来。长椅下压着一只维修包，远处列车尾灯像两点不会靠近的红光。', targets: [{ id: 'maintenance-cart', name: '轨旁维修小车', observation: '小车停在屏蔽门外半米处。可以用绳索拖回，也可以冒险跨下站台。', options: [
      { id: 'rope', label: '用绳索套住车把拖回', hint: '35分钟 · 体力 -7 · 搜寻经验 +2 · 安全', result: '绳圈第三次才套住车把。小车轮沿轨旁槽缓慢滑回站台。', minutes: 35, stamina: 7, requirements: [{ item: 'rope' }], loot: { multitool: 1, 'copper-wire': 1, batteries: 1 }, skillXp: { search: 2 } },
      { id: 'climb-down', label: '跨下站台直接推车', hint: '20分钟 · 体力 -13 · 高风险', result: '你刚把小车推到门边，隧道深处就传来钢轮滚动般的回声。', minutes: 20, stamina: 13, loot: { multitool: 1, batteries: 1 }, skillXp: { toolUse: 1 }, danger: 42 },
    ] }] },
...
const NORTH_SUBSTATION: DeepLocation = {
  ...
  scenes: [
    { id: 'perimeter', name: '围栏与门卫室', connections: ['switchyard', 'lobby'], text: '围栏上挂着褪色的高压警示牌，门卫室玻璃没有破。访客登记本停在封锁前一晚，控制楼门禁仍靠备用电发着红光。', targets: [{ id: 'guard-cabinet', name: '门卫应急柜', observation: '柜中有绝缘手套、雨衣和访客路线图。锁不复杂，但门卫室地板满是碎玻璃。', options: [
      ...
      { id: 'quick', label: '踩过碎玻璃直接取物', hint: '15分钟 · 体力 -7 · 有割伤风险', result: '柜锁被门框挤开了一半。你够到雨衣，却不敢在碎玻璃上停太久。', minutes: 15, stamina: 7, loot: { raincoat: 1 }, skillXp: { search: 1 }, danger: 24 },
    ] }] },
    { id: 'switchyard', name: '露天开关场', connections: ['perimeter', 'cable-room'], text: '瓷瓶和母线在头顶排成冷硬的几何形。远处一组应急蓄电柜仍有电压，积水却已经漫到检修步道边缘。', targets: [{ id: 'battery-bank', name: '站用蓄电柜', observation: '电芯能为避难所补电，但拆错顺序会造成短路。柜门贴着检修流程，最后三步被雨泡糊了。', options: [
      { id: 'procedure', label: '按线路图隔离后拆取电芯', hint: '75分钟 · 体力 -9 · 工具箱、情报 ≥ 2 · 电力 +12', result: '你逐段确认无压，拆下一组仍健康的电芯，端子没有冒出火花。', minutes: 75, stamina: 9, requirements: [{ item: 'toolkit' }, { minIntel: 2 }], loot: { batteries: 2, 'copper-wire': 1 }, skillXp: { toolUse: 3 }, effects: { shelter: { power: 12 } } },
      { id: 'quick-disconnect', label: '用绝缘布快速断开端子', hint: '35分钟 · 体力 -13 · 高风险 · 电力 +6', result: '扳手碰到柜壁时蓝光一闪。你拖走半组电芯，剩下的已经保护跳闸。', minutes: 35, stamina: 13, requirements: [{ item: 'gloves' }], loot: { batteries: 1 }, skillXp: { toolUse: 2 }, effects: { shelter: { power: 6 } }, danger: 41 },
    ] }] },
