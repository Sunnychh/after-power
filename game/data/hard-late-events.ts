...
export const HARD_LATE_EVENTS: GameEvent[] = [
  {
    ...
    difficulties: ['hard'], inventoryAny: ['water-bottle'], hardStockPressure: true, npc: 'chen-meng',
    text: '连续围攻后，楼里还能走动的人把空桶排在消防门旁。陈檬在每只桶上写了门牌：有老人、有孩子，也有两户已经一天没人回应。她没有问你有多少水，只把名单递过来。',
    options: [
      { label: '拿出两瓶水补公共配给', hint: '瓶装水 -2，陈檬信任 +10，精神 +5', result: '两瓶水被分成六份。没人喝饱，但夜间值守表上多了三个愿意出门的名字。', requirements: [{ item: 'water-bottle', quantity: 2 }], effects: { inventory: { 'water-bottle': -2 }, relationships: { 'chen-meng': 10 }, stats: { morale: 5 }, addFlags: ['late-water-shared'] } },
      { label: '只给楼梯口值守者一瓶', hint: '瓶装水 -1，情报 +1，陈檬信任 +4', result: '值守者把水分着喝完，也把今晚感染群聚集的楼层告诉了你。', requirements: [{ item: 'water-bottle', quantity: 1 }], effects: { inventory: { 'water-bottle': -1 }, relationships: { 'chen-meng': 4 }, intel: 1 } },
      { label: '说明自己的储备也撑不到撤离', hint: '物资不变，精神 -4', result: '陈檬把你的门牌从名单上划掉。她没有指责，只把空桶继续搬向下一层。', effects: { stats: { morale: -4 } } },
    ],
  },
  {
    id: 'hard-food-watch', title: '守夜班缺少的一顿饭', phase: 'survival', minDay: 9, maxDay: 14,
    difficulties: ['hard'], inventoryAny: ['canned-beans', 'crackers'], hardStockPressure: true, npc: 'chen-meng',
    text: '负责敲击管道示警的人已经连守两夜。今天没人来接班，因为公共食物箱只剩包装纸。楼道里的人知道你可能还有耐储食物，但没有人直接敲门讨要。',
    options: [
      { label: '交出一罐豆子作为守夜口粮', hint: '豆子罐头 -1，加固 +1，精神 +3', result: '罐头在值守者之间传了一圈。吃完后，他们把松动的消防门重新顶住。', requirements: [{ item: 'canned-beans', quantity: 1 }], effects: { inventory: { 'canned-beans': -1 }, shelter: { reinforcement: 1 }, stats: { morale: 3 }, relationships: { 'chen-meng': 5 } } },
      { label: '拿一包饼干换一份街面观察', hint: '咸味饼干 -1，情报 +2', result: '年轻值守者边吃边画出尸群移动方向。下一次外出至少不会迎面撞上主潮。', requirements: [{ item: 'crackers', quantity: 1 }], effects: { inventory: { crackers: -1 }, intel: 2, relationships: { 'chen-meng': 3 } } },
  ...
  },
  {
    ...
    difficulties: ['hard'], inventoryAny: ['bandage', 'disinfectant'], hardStockPressure: true, npc: 'lin-zhou',
  ...
  },
  {
    ...
    options: [
      { label: '交出一组电池维持门磁', hint: '电池 -1，加固 +1，潘岳信任 +5', result: '门磁重新逐层亮起。它们挡不住撞击，却能让所有人提前站到正确的位置。', requirements: [{ item: 'batteries', quantity: 1 }], effects: { inventory: { batteries: -1 }, shelter: { reinforcement: 1 }, relationships: { 'pan-yue': 5 }, addFlags: ['late-alarm-powered'] } },
      { label: '把一罐燃料送到地下机房', hint: '密封燃料罐 -1，储水 +8，潘岳信任 +8', result: '发电机带动警戒灯，也让水泵多运行了几分钟。潘岳把接出的水优先送回你门口。', requirements: [{ item: 'fuel-can', quantity: 1 }], effects: { inventory: { 'fuel-can': -1 }, shelter: { water: 8 }, relationships: { 'pan-yue': 8 } } },
  ...
  },
  {
    ...
    difficulties: ['hard'], inventoryAny: ['wood-board', 'metal-sheet'], hardStockPressure: true, npc: 'pan-yue',
