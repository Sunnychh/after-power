...
export const CONTACTS: ContactDefinition[] = [
  {
    ...
    options: [
      { id: 'consult', label: '询问楼栋与街面情况', hint: '情报 +1，信任 +2', result: '陈檬按楼层报出安全通道、缺口和仍有人回应的门牌。你第一次看见这栋楼作为一个整体如何活着。', effects: { intel: 1, relationships: { 'chen-meng': 2 } } },
  ...
  },
  {
    ...
    options: [
      { id: 'consult', label: '请她远程检查当前状态', hint: '精神 +1，信任 +2（不替代药品）', result: '她让你依次检查体温、伤口边缘和呼吸频率，并纠正了两处容易忽略的处理细节。远程诊断不能让伤口凭空愈合，但至少让你知道下一步该做什么。', effects: { stats: { morale: 1 }, relationships: { 'lin-zhou': 2 } } },
  ...
  },
  {
    ...
    options: [
      { id: 'consult', label: '核对感染与封锁记录', hint: '情报 +1，信任 +2', result: '邱岚把三条互相矛盾的通报拆开比对。她不提供安慰，只留下可以验证的时间和编号。', effects: { intel: 1, relationships: { 'qiu-lan': 2 } } },
  ...
  },
  {
    ...
    options: [
      { id: 'consult', label: '让他听门框与管线异响', hint: '情报 +1，信任 +2（不生成材料）', result: '你把收音机贴近门框。潘岳判断出下一处应该处理的受力点，但提醒你：没有胶带、木板或钢件，知道位置也不能把裂口修好。', effects: { intel: 1, relationships: { 'pan-yue': 2 } } },
